...
void pre_auton(void) {
  ...
  Brain.Screen.setFillColor(black);
  Brain.Screen.print("jwashers on the drivetrain");
...
Drivetrain.driveFor(forward,10,inches);*/
  ...
  Drivetrain.driveFor(forward, 10, inches);
   ...
   piston.set(true);
   piston_2.set(true);
   poncher.spinFor(reverse,4,turns);
   //poncher.spinFor(reverse,48,turns);//
   piston.set(false);
   piston_2.set(false);
...
void usercontrol(void) {
 ...
 /*while (true) {
    ...
    {
      //controller1.ButtonL2.poncher.spin(reverse)
    wait (20,msec);
    }
  /*poncher.setVelocity(100,percent);
  while (true) {
  poncher.setVelocity(100,percent);
  if (Controller1.ButtonL1.pressing()) 
    poncher.spinFor(reverse, 90, degrees);*/
  /*}
  while (true) {
